...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Validating-Machine thinks for you about the meanings.",
        epilog="Example:  validating-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | validating-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | validating-machine         #

    $ validating-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="validating-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"validating-machine: {utterance}.")
        syslog.closelog()
